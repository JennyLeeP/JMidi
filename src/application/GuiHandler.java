...
import application.actions.MultiKeyPressAction;
import application.actions.PlayWaveAction;
...
public class GuiHandler {
	...
	private void ruleSetModal(){
		...
		create.setOnAction(e -> {  //TODO Change if's to be in series not nested.
			...
			} else if (actionDrop.getSelectionModel().getSelectedItem().equals("Wave Sound")) {
				if (!path.getText().isEmpty()) {
					addControlRow(name.getText(), buttonInput.getText(), actionDrop.getValue(), fileName);
					ActionMap.addAction(stringToInt(buttonInput.getText()), new PlayWaveAction(filePath));
					dialog.close();   
				} else {
	...
	private void addControlRow(String name,String butId, String actionType, String action) {
		...
		deleteRow.setOnAction(e -> {
			...
			ActionMap.removeAction(stringToInt(bval.getText()));
		});
	}
}
